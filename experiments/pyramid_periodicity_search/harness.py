...
pipeline.localize.localize except a set of coarse-to-fine pyramid
candidates (pyramid_search.build_coarse_to_fine_candidates) are merged
into the raw candidate pool BEFORE deduplication. Candidate generation's
own 99 grid hypotheses (at full resolution), dedup, classical ranking,
...
from pyramid_search import build_coarse_to_fine_candidates


def localize_pyramid(reference_img, search_img, *,
                      downsample_factor: float = 4.0,
                      top_k_coarse: int = 3,
                      window_margin_px: int = 15,
    ...
    pyramid_candidates = build_coarse_to_fine_candidates(
        reference, search, downsample_factor=downsample_factor, top_k_coarse=top_k_coarse,
        window_margin_px=window_margin_px, scale_hypotheses=scale_hypotheses,
        rotation_hypotheses=rotation_hypotheses,
    )

    fused_raw = list(raw_candidates) + list(pyramid_candidates)
    ...
    return LocalizationResult(
        ...
        runtime_s=runtime_s, ranking_mode="classical+pyramid_periodicity", num_candidates=len(candidates),
        top_candidates=[
