...
| B3 | Justify structures/noise/augmentations against ≥2–3 credible public sources; **cited in the PPT** and documentation | Help doc §4B ("include citations in the PPT and documentation") | ⚠️ **Documentation side now closed (2026-08-17):** `references/` holds a verified bibliography — 21 entries, every one carrying a DOI, stable URL or ISBN, each mapped to the file and function implementing it, plus `references/BIBLIOGRAPHY.bib`. Structures are now cited (Keeth et al. 2007; Vogelsang 2010; Kim et al. 2012; Itoh 1990/2001), which was previously the largest hole — the mat/strip claim had no source at all. 14 of the 22 degradation rows now carry a mechanism-specific citation, 6 are labelled standard-model with a textbook anchor, 2 are not physical mechanisms. **STILL OPEN:** the submission PPT's References slide (slide 9) remains placeholder text (`{Ref 1: Title...}`); the spec calls out the PPT specifically. Paste from `references/README.md` |
