#!/usr/bin/env python
"""Two-stage, honest evaluation of local peak-prominence re-ranking:

  1. Sweep (gamma, top_k) on `development` ONLY (24 pairs) - never on
     validation/held_out/challenge/cross_generator, matching this
     project's established dev-only-tuning discipline. annulus radii held
     fixed at the values tuned via direct radial-profile measurement
     (inner_exclude_px=2.0, annulus_outer_px=12.0 - see prominence.py).
...
An exploratory probe at gamma=15 (well outside this sweep's range) showed
this signal is NOT inert like several prior experiments - it produced one
genuine catastrophic rescue (dev_dense_periodic_000: 525px -> 41px) but
also broke several previously-near-perfect pairs (e.g. dev_strip_anchor_003:
0.31px -> 39.3px) - net effect at that strength was clearly negative. This
sweep checks whether a smaller gamma finds a net-positive operating point.
...
def _run_split(split: str, *, gamma: float, top_k: int) -> pd.DataFrame:
    ...
    for _, row in manifest.iterrows():
        ...
        result = localize_prominence(ref, search, gamma=gamma, top_k=top_k)
    ...
    for gamma in SWEEP_GAMMA:
        for top_k in SWEEP_TOPK:
            if gamma == 0.0 and top_k != SWEEP_TOPK[0]:
                continue  # gamma=0 collapses to rank_classical regardless of top_k - only run once
            ...
            print(f"  gamma={gamma:<5} top_k={top_k:<3} acc@5px={acc5:.3f} mean_err={mean_err:8.2f}px")
...
def run_frozen_benchmark(gamma: float, top_k: int) -> pd.DataFrame:
    ...
    combined.to_csv(os.path.join(OUT_DIR, "per_pair_results_prominence.csv"), index=False)
    return combined
...
def main() -> None:
    ...
    with open(os.path.join(OUT_DIR, "prominence_metrics.json"), "w") as f:
