...
pipeline.localize.localize except the refinement step is swapped for
refine_joint - candidate generation, dedup, classical ranking, and center
tiebreak are all the UNMODIFIED production functions, called directly.
"""
...
from joint_refine import refine_joint


def localize_joint_refine(reference_img, search_img, *,
    ...
    winner = ranked[0]
    refined_x, refined_y, _refined_scale, _refined_rotation = refine_joint(
        reference, search, winner, scale_hypotheses=scale_hypotheses, rotation_hypotheses=rotation_hypotheses,
    )
    ...
    return LocalizationResult(
        ...
        runtime_s=runtime_s, ranking_mode="classical+joint_refine", num_candidates=len(candidates),
        top_candidates=[
