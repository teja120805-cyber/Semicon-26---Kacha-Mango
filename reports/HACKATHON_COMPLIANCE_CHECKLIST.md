...
| E8 | No proprietary data or hard-coded local paths | Help doc §9 checklist | ⚠️ this row previously read ✅, which was false: **30 hard-coded `/tmp/driftsense` literals across 23 experiment scripts** were still present (on top of the one path fixed during repo cleanup, `reports/PROJECT_STATUS.md` Phase 7). All 30 were found and fixed in this pass (2026-08-17). **OPEN:** `outputs/reports/per_pair_results.csv` still stores absolute Windows paths in its `reference_path` / `search_path` columns, because `evaluation/evaluate.py` joins whatever absolute `--data-root` it is given — the CSV must be regenerated with a relative `--data-root`, or the columns made relative, before packaging. No proprietary data anywhere: every image is generated by `generator/` |
