#!/usr/bin/env python
"""Two-stage, honest evaluation of pitch-aware prominence re-ranking:
...
SWEEP_GAMMA = [0.0, 1.0, 3.0, 5.0, 8.0, 12.0]
SWEEP_TOPK = [4, 8]
...
def _run_split(split: str, *, gamma: float, top_k: int) -> pd.DataFrame:
    ...
    for _, row in manifest.iterrows():
        ...
        result = localize_pitch_aware(ref, search, gamma=gamma, top_k=top_k)
        ...
        rows.append({
            **row.to_dict(),
            ...
            "ranking_mode": "pitch_aware_prominence",
        })
...
def run_frozen_benchmark(gamma: float, top_k: int) -> pd.DataFrame:
    ...
    combined.to_csv(os.path.join(OUT_DIR, "per_pair_results_pitch_aware.csv"), index=False)
    return combined
...
def main() -> None:
    ...
    with open(os.path.join(OUT_DIR, "pitch_aware_metrics.json"), "w") as f:
    ...
    # Direct rescue/break diagnostics vs baseline (not just aggregate accuracy).
    ...
    print(f"\nRescued across 5px line: {rescued}  |  Broken across 5px line: {broken}")
