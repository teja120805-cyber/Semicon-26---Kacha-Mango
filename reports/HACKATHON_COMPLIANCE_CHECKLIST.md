...
| B3 | Justify structures/noise/augmentations against ≥2–3 credible public sources; **cited in the PPT** and documentation | Help doc §4B ("include citations in the PPT and documentation") | 🔴 sources exist and are solid (Foi et al. 2008, Orji et al. 2018 *Nature Electronics*, the AM/HuggingFace reference generator) and are cited throughout `reports/`, but the actual submission PPT's References slide (slide 9) is still placeholder text (`{Ref 1: Title...}`) — needs to be filled in before submission, the spec calls out the PPT specifically, not just supporting docs |
...
| D3 | Pass rate at 5-, 4-, 2-, 1-px thresholds + sub-pixel where supported | Help doc §4D, pptx slide 8 | ✅ `metrics.py` computes all four; median error 0.33px demonstrates real sub-pixel performance |
| D4 | Mean, median, worst-case error | Help doc §4D | ✅ |
| D5 | Runtime per pair, with hardware, Python version, and timing method stated | Help doc §4D | ⚠️ `runtime_s` is measured correctly in code — verify the final README/PPT explicitly states the hardware spec and Python version used, not just the number |
...
| D7 | ≥1 visualized failure case with root-cause explanation | Help doc §4D | ✅ `outputs/visualizations/catastrophic_failures/` + written root-cause analysis |

## E. Deliverables / submission — hard requirements

| # | Requirement | Source | Status |
|---|---|---|---|
| E1 | Mandatory solution PPT/PPTX | Help doc §5, §7 | 🔴 current deck (`Kaccha Mango_PS02.pptx`) is the shorter "idea submission" shape, not yet the fuller 12-slide "solution PPT" structure the help doc recommends (architecture, per-threshold results, robustness/ablation, failure case, limitations) |
...
| E3 | README: env setup, folder structure, exact commands, I/O examples, coordinate convention, assumptions | Help doc §5 | ✅ |
...
| E7 | References doc: public sources for structures/imaging/noise/transforms | Help doc §5 | ⚠️ solid in `reports/`, but same as B3 — make sure it's also front-and-center in the PPT itself |
| E8 | No proprietary data or hard-coded local paths | Help doc §9 checklist | ✅ already found and fixed one hard-coded path during repo cleanup (`reports/PROJECT_STATUS.md` Phase 7) |
...
numbers. Added as its own README section ("Evaluation criteria alignment") and surfaced live in the
Streamlit app (a badge on each relevant page + a table on Executive Summary) on 2026-08-15, so a
judge skimming either artifact can see the mapping without reading this file.
